...
function General({ category }) {
  ...
  return (
    <div className="py-12 min-h-[52vh] px-16 font-[QuickSand] text-slate-700 dark:text-white transition-all duration-500 dark:bg-zinc-700">
      ...
      {title.pagination.length ? (
        <div className="flex w-full items-center justify-center gap-12 mt-12">
          <a href={`${window.location.pathname.replace(currentPage.toString(), parseInt(currentPage, 10) - 1 >= 1 ? parseInt(currentPage, 10) - 1 : 1)}`} aria-label="hidden"><Icon icon="uil:angle-double-left" className="w-8 h-8 text-amber-400" /></a>
          {title.pagination.slice(1, title.pagination.length - 1).map((e) => (
            <a href={`${window.location.pathname.replace(currentPage.toString(), e)}`} className={`${e === currentPage ? 'bg-amber-400 text-white p-4 rounded-md shadow-md w-12 h-12 flex items-center justify-center' : 'text-slate-700'} font-medium text-xl`}>
              {e}
            </a>
          ))}
          <a href={`${window.location.pathname.replace(currentPage.toString(), parseInt(currentPage, 10) + 1)}`} aria-label="hidden"><Icon icon="uil:angle-double-right" className="w-8 h-8 text-amber-400" /></a>
        </div>
      ) : ''}
    </div>
  );
