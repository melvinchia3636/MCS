...
const Section = ({data, title, desc}) => {
    return <div className="py-12 font-[QuickSand] text-slate-700 dark:text-white transition-all duration-500 dark:bg-zinc-700">
        <h1 className="font-medium text-4xl mb-3">{title}</h1>
        <p className="text-lg tracking-wide">{desc}</p>
        <div className="grid grid-cols-2 gap-6 mt-8">
            {data.length ? data.map(e => <div className="w-full overflow-hidden rounded-xl shadow-[0_6px_12px_rgba(0,0,0,.15)] bg-white dark:bg-zinc-600 transition-all duration-500">
                <div className="p-6">
                    <div className="flex items-center justify-between">
                        ...
                        <div className={`flex items-center gap-1 rounded-full ${e.status === "Online" ? "bg-amber-400" : "bg-rose-500"} pr-4 pl-3 font-medium py-1 shadow-sm text-white`}><Icon icon={"uil:"+(e.status === "Online" ? "check" : "times")} className="mt-0.5 w-5 h-5" />{e.status}</div>
                    </div>
                    <div className="flex flex-col gap-6 mt-8">
                        ...
                        ].map(e => <div className="flex px-2">
                            {e.map(([i, e]) => <div className="w-1/2 text-lg flex items-center gap-2"><Icon icon={"uil:"+i} className="text-amber-400 w-6 h-6" />{e}</div>)}
                        </div>)}
                    </div>
                </div>
...
const Home = () => {
    ...
    return data.length ? <div className="flex flex-col mx-16">
        <Section data={data[0]} title={<><span className="text-amber-400">The Best</span> Minecraft Servers</>} desc={<>Looking for the <b>best Minecraft servers</b>? We have over 1050 of them! You can find all sorts of game modes and versions. Whether you're looking for a survival server, a creative server, or even one with PvP - we've got it covered.</>} />
