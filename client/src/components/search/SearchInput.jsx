    ...
    <>
      {mode === null ? (
        <>
          ...
          <p className="text-xl tracking-wide mb-8 text-slate-600">What would you like to do? :)</p>
          <div className="flex w-full items-center justify-center gap-4">
            <button onClick={() => setMode(0)} type="button" className="w-full h-16 flex items-center justify-center gap-2 bg-slate-100 shadow-md font-medium text-xl text-slate-700 rounded-md">
      ...
      {mode === 0 ? (
        <div className="h-full flex-grow w-full flex flex-col justify-center gap-4">
          ...
          <p className="text-xl tracking-wide mb-4 text-slate-600">
            Tell us what you&apos;re looking for and we&apos;ll find it ❤️
          </p>
          <div className="shadow-md p-6 rounded-md text-xl overflow-hidden flex items-center justify-center w-full gap-2">
            <Icon icon="uil:search" width="28" height="28" className="text-slate-400" />
            <input onChange={(e) => setQuery(e.target.value)} onEnter={gotoSearchResult} type="text" placeholder="Type your keyword here..." className="w-full tracking-wide text-slate-700 focus:outline-none" />
          </div>
