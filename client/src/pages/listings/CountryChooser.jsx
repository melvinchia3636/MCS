...
function CountryChooser({
  ...
  return (
    <div className={`${isCountryChooserOpen ? 'z-[9999] bg-opacity-10' : 'z-[-1] bg-opacity-0'} fixed w-full h-screen top-0 left-0 flex items-center justify-center bg-black transition-all duration-500`}>
      <div className={`transform ${isCountryChooserOpen ? 'translate-y-0' : 'translate-y-[200%]'} bg-white dark:bg-zinc-600 w-[28rem] h-[calc(100vh-8rem)] shadow-lg py-6 px-8 text-slate-700 dark:text-white rounded-xl transition-all duration-500 flex flex-col`}>
